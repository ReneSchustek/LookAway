...
/// <summary>
/// Zulaessiger Bereich der Arbeitsdauer eines Modells, das eine begrenzte
/// Benutzer-Anpassung erlaubt (z. B. <c>PhysicalCounter</c> 30–45 Minuten).
/// Vorgesehen als Grenzen fuer einen UI-Slider.
/// </summary>
/// <remarks>
/// Kleiner, unveraenderlicher Wertebereich — als <c>readonly record struct</c>
...
public readonly record struct WorkDurationRange
{
    ...
    /// <exception cref="ArgumentException"><paramref name="min"/> ist groesser als <paramref name="max"/>.</exception>
    public WorkDurationRange(TimeSpan min, TimeSpan max)
    {
        ...
        if (min > max)
        {
            throw new ArgumentException($"Min ({min}) darf nicht groesser als Max ({max}) sein.", nameof(min));
        }
